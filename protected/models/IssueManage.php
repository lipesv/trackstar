...
class IssueManage extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('id, project_id, requester_id, owner_id', 'numerical', 'integerOnly'=>true),
			array('issue, project, requester, owner', 'length', 'max'=>255),
			array('type_id, status_id', 'length', 'max'=>11),
			array('description', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, issue, description, project_id, project, type_id, status_id, requester_id, requester, owner_id, owner', 'safe', 'on'=>'search'),
		);
	}
